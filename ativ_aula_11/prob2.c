...
typedef struct {
    char nome[NSIZE]; //primeiro e segundo nome 
    char nomeEquipa[TSIZE]; //nome da equipa
    int jogos; //numero de jogos
    float min, pts; //min=media min jogados e pts=media de pontos marcados
} jogador;
...
int gravaEstatisticas(jogador statsJogadores[], int n, char *equipa, char *nomeFicheiro) {
    int i, cont=0;
    char nomeficheiro[NSIZE];
    FILE *f;
    f=fopen(nomeFicheiro, "w");

    for(i=0; i<n; i++) {
        if(strcmp(statsJogadores[i].nomeEquipa, equipa)==0) {
            fprintf(f, "%s %s ", statsJogadores[i].nome, statsJogadores[i].nomeEquipa);
            fprintf(f, "%d %.1f %.1f\n", statsJogadores[i].jogos, statsJogadores[i].min, statsJogadores[i].pts);
            cont++;
        }
    }
...
void top10Jogadores(jogador statsJogadores[], int n, jogador topJogadores[]) {
    ...
    //1º passo -> ordenar o vetor por ordem decrecente
    for(i=0; i<n; i++) {
        for(j=i; j>0; j--) {
            if(statsJogadores[j].pts>statsJogadores[j-1].pts) {
                aux=statsJogadores[j];
                statsJogadores[j]=statsJogadores[j-1];
                statsJogadores[j-1]=aux;
            }
        }
    }

    //2º passo -> copiar os 10 primeiros valores do vetor ordenado para topJogadores
    for(i=0; i<10; i++) {
