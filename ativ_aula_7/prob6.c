...
void filtro_media(unsigned char entrada[], unsigned char saida[], int largura, int altura)
{
    ...
    for(j=0; j<altura; j++)
    {
        for(i=0; i<largura; i++)
        {   
            if((i!=0) && (j!=(altura-1)) && (j!=0) && (i!=(largura-1)))
            {
                saida[POS(i, j)]=(entrada[POS(i-1, j-1)]+entrada[POS(i, j-1)]+entrada[POS(i+1, j-1)]+entrada[POS(i-1, j)]+entrada[POS(i, j)]+entrada[POS(i+1, j)]+entrada[POS(i-1, j+1)]+entrada[POS(i, j+1)]+entrada[POS(i+1, j+1)])/9;
            }
        }
    }
}
...
void filtro_sobel(unsigned char entrada[], unsigned char saida[], int largura, int altura)
{
    ...
    for(j=0; j<altura; j++)
    {
        ...
        {   
            if((i!=0) && (j!=(altura-1)) && (j!=0) && (i!=(largura-1)))            
            {
                k=-entrada[POS(i-1, j-1)]+entrada[POS(i+1, j-1)]-2*entrada[POS(i-1, j)]+2*entrada[POS(i+1, j)]-entrada[POS(i-1, j+1)]+entrada[POS(i+1, j+1)];
                if(k>=0) saida[POS(i, j)]=k;
                else saida[POS(i, j)]=-k;
            }
        }
    }
}
...
void filtro_threshold(unsigned char entrada[], unsigned char saida[], int largura, int altura, int threshold)
{
    ...
    for(j=0; j<altura; j++)
    {
        for(i=0; i<largura; i++)
        {   
            if(entrada[POS(i, j)]<threshold) saida[POS(i, j)]=0;
            else saida[POS(i, j)]=255;
        }
    }
}
